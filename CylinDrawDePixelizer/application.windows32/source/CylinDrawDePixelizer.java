...
public class CylinDrawDePixelizer extends PApplet {
...
float matrix_multiplier = 0.0f;      // multiplication factor used in spatial convo processing
...
int cOld; 
int c01;//what the pixel wouyld have been if not dithered
int backgroundColor;
int    iColorLimit = 2;
int    iColorLimitOld = 0;
int  c00;
int[]  colorArray =  new int[19];  //array of just the unique colors!
...
float[] colorPosArray =  new float [19];
  ...
  int cPicked = color(255,255,255); //user picked color
  Color javaColor;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
public void draw(){// println(frameRate);
  ...
  if ( toolDia != toolDiaOld || imageLoaded ){
      ...
      for (int y = 1; y < PixelHeight; y+= matrixsize ) { //LOOOP TOP TO BOTTOM
        ...
        if (y % 2==0){  // loop LEFT TO RIGHT through the matrix, applying the matrix filter to the pixels
          ...
          for (int x = 1; x < PixelWidth; x+= matrixsize) {
            ...
            bPrevHit = hit;
            hit = false;
            int c = convolution(x, y, matrixsize);
            if (x==1) {cOld =c; }
   ...
   if (recordingData){    //this is where I want to trigger the 'finished' pop up....
           ...
           float textSize = 0.5f*(100/(100-overLap))* RealHeight/toolDia/11.75f  ;//17 is generall but want to be smaller for low pixel count images (high tool dia, low image size)(  RealHeight/toolDia  100/.5 =200
           float textSpacing = 0.5f*(100/(100-overLap))* RealHeight/toolDia/10;//20 is general.
...
public void fileCopy(){  //If user picks file not located in processing parent folder, we copy it temporarily, then delete that copy on proper exit. File  
...
public void reloadImage(String path){ // set up the new image and reset resolution// called when user loads new image-
   if(fileLoaded){
    filePath = path;
    ...
    loadPixels(); ///WHOOOO leave this here, it cures the phantom issue from when you change the screen!
     ...
     if (recordingData){
      ...
      //    PixelWidth = (int)(img.width*zoomScale);
          ...
          while ( (PApplet.parseInt(img.height*zoomScale)) >= zLimit-57 ){ //this caps off the max size of the image you can zoom to
             zoomScale = zoomScale -.01f;
           }   
      }
      ...
      if(toolDia<=0){
           toolDia=0;
           NumStrokes = round( (RealHeight/(.01f))  );// this will give the tightest slice possible!
      }else{
    ...
    if (iPoster>0)  { 
      int val = PApplet.parseInt(map(iPoster,1,100,11,2));
...
public void drawUI(){ // show text instructions for use  
  
   float downscale = 2.6f;int imgpixelwidth = 2030;int imgpixelheight = 244;imageMode(CENTER);
...
public void mouseWheel(MouseEvent event){
  if (imageLoaded){
    ...
    if (zoomScale<.01f){zoomScale = .01f;}
...
public void buttonProgConvert(){println("You are currently using DePixelizer Mode.");}
    



public void buttonProgCreate(){ 
...
public void buttonToolUp(){     toolDia = toolDia +.05f;  imageLoaded = false;   reloadImage(filePath); }
public void buttonToolDown(){   toolDia = toolDia -.05f;   if (toolDia <=0){toolDia = .05f;}  imageLoaded = false;   reloadImage(filePath); }
public void buttonSizeUp(){   RealHeight = RealHeight +1; imageLoaded = false;   reloadImage(filePath); }
public void buttonSizeDown(){   RealHeight = RealHeight -1; if (RealHeight <10){RealHeight = 10;} imageLoaded = false;   reloadImage(filePath); }
public void toggleInverted(){  inverted = toggleInverted.getBooleanValue();;  imageLoaded = false;   reloadImage(filePath); }//inverted = toggleInverted.getBooleanValue(); 
public void toggleBlur(){    filterBlur =  toggleBlur.getBooleanValue();;  imageLoaded = false;   reloadImage(filePath); }
public void toggleTrace(){  filterTrace =  toggleTrace.getBooleanValue();  imageLoaded = false;   reloadImage(filePath); }
public void toggleSharpen(){  filterSharpen =  toggleSharpen.getBooleanValue();  imageLoaded = false;   reloadImage(filePath); }

public void toggleErode(){  filterErode = toggleErode.getBooleanValue();  imageLoaded = false;   reloadImage(filePath); }
public void toggleDilate(){   filterDilate =  toggleDilate.getBooleanValue();  imageLoaded = false;   reloadImage(filePath); }
public void toggleSpeckle(){  filterSpeckle =  toggleSpeckle.getBooleanValue();  imageLoaded = false;   reloadImage(filePath); }
public void toggleDither(){  bDither =  toggleDither.getBooleanValue(); if(bDither){overLap=0;}else{overLap=50;} imageLoaded = false;   reloadImage(filePath); }
public void toggleBackground(){ filterBackground =  toggleBackground.getBooleanValue();;  imageLoaded = false;   reloadImage(filePath); }
...
public float colorPosition(int c1){ //min value is 0 max is 441.673 when colors are constrained between 0-255
  float r = red(c1);
  float g = green(c1);
  float b = blue(c1);
  return sqrt( sq(r) + sq(g) + sq(b) );  // return  dist(red(c1), red(c2),green(c1),green(c2), blue(c1),blue(c2));
} 
public float colorVariance(int c1){////tells you how not-gray a color is. (higher yoyoutput isl less gra)  //min value is 0 max is 441.673 when colors are constrained between 0-255
  float r = red(c1);
  float g = green(c1);
  float b = blue(c1);
...
public void sliderThresholdLow(int value){
...
public void sliderThresholdHigh(int value){
  if (imageLoaded  && !bSliderLock){
...
public void sliderOverlap(int value){ //note this exists but is hidden from the user, I dont wantthem controlling it
  if (imageLoaded  && !bSliderLock){ overLap = value; imageLoaded = false;   reloadImage(filePath);}
}

public void sliderPoster(int value){
...
public int sharpen(int x, int y, PImage imgs) {
  float[][] kernel ={
    { -1, -1, -1}, 
    { -1, 9, -1}, 
    { -1, -1, -1}
...
public void checkFrequency( int colorIn){
...
public int convolution(int x, int y, int matsize) { // uses a spatial matrix technique to apply a smoothing of pixels in a given area. // averaging the pixel colour to produce the next image
    float rtotal = 0.0f;
    float gtotal = 0.0f;
    float btotal = 0.0f;
    float rorig = 0.0f;
    ...
    matrix_multiplier = 1.0f / num_pixels;
    //println("multiplier " + matrix_multiplier);
    int c00 = (255/2);// = color(rtotal, gtotal, btotal);
...
public void sliderColors(int value){ // input limits: 2<=value<=12
  if (value <2 || value >12 || bSliderLock){return;}
  
   iColorLimit = PApplet.parseInt( sliderColors.getValue()); 
   ...
   if (value >0 &&  imageLoaded  ){
      ...
      for (int rev =1; rev<=iColorLimit; rev++){ //rev =0 originally
         ...
         int val = PApplet.parseInt(map(rev,1,iColorLimit,0,255)); //DEFAULT!!
         int r = val; 
         int g = val;
         int b = val;//+1; why did i add this?
         //b = constrain(b, 0, 255);
         int rrr= color(r,g,b);//,g,b);//,g,b);
         
         if (iColorLimit ==2){
           val = PApplet.parseInt(map(rev,1,iColorLimit,thresholdLow,thresholdHigh)); //DEFAULT!!
           r = val; 
           g = val;
           b = val;
           rrr= color(r,g,b);
...
public void keyPressed(){  if (key == ESC) { key = 0; buttonExit();} }
...
public void setButtons(){
  ...
  bSliderLock = false;
   ...
   inverted = toggleInverted.getBooleanValue();
          ...
          iPoster= PApplet.parseInt(sliderPoster.getValue());  //iPoster
          //brightness= int(sliderBrightness.getValue());
         // contrast= int(sliderContrast.getValue());
          iColorLimit = PApplet.parseInt( sliderColors.getValue()); //iColorLimit
          thresholdLow = PApplet.parseInt( sliderThresholdLow.getValue());
          thresholdHigh = PApplet.parseInt( sliderThresholdHigh.getValue());
}

public void showHideButtons(boolean bHideAll){
...
public int colorPicker(int input){
...
/*long currentTime(){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "CylinDrawDePixelizer" };
    if (passedArgs != null) {
