...
public class CylinDrawViewer extends PApplet {
...
float xValue=0, yValue=0, zValue=0.2f, lastX = xShift, lastY = yShift, offsetX=0, offsetY=0;
...
int track[] = {color(0, 0, 0), color(170, 0, 0), color(0, 192, 0), color(40, 40, 255) };
Color wild;
int wild2;
...
public void draw() {
  ...
  if (bReset){
        ...
        xValue=0; yValue=0; zValue=0.2f; lastX = xShift; lastY = yShift; offsetX=0; offsetY=0; iValue = 0; jValue = 0;
  ...
  imageMode(CENTER);
  float downscale = 2.2f;   int imgpixelwidth = 1690;int imgpixelheight = 248;
...
public void draw_Zero() {  // draw the zero axis
  stroke(255, 255, 0);
  ...
  for (int i = 0; i < xWindow; i++) { //X AXIS
    line(xShift + (PApplet.parseFloat( i) * plotScale), yShift, xShift +( PApplet.parseFloat(i) * plotScale), yShift - 3);
    if ((i % 10) == 0) line(xShift + (PApplet.parseFloat( i) * plotScale), yShift, xShift +( PApplet.parseFloat(i) * plotScale), yShift - 7);
  } 
  for (int i = 0; i < yWindow; i++) { //Y AXIS
    line(xShift, yShift - (PApplet.parseFloat(i)* plotScale), xShift + 3, yShift - (PApplet.parseFloat(i)* plotScale) );
    if ((i % 10) == 0) line(xShift, yShift - (PApplet.parseFloat(i)* plotScale), xShift + 7, yShift - (PApplet.parseFloat(i)* plotScale) );
  }
}


public void   loadGcode() { //HAPPENS EVERY CYCLE, 
...
public void gatherCommand(int i, int j) {   // gets the next paramater from the line
  float x, y, z;
  switch(lines[j].charAt(i)) {
      case'M': //Now its 'M0 '//QQ  this has to be a SINGLE letter that it recognizes by and thats why its a single Q.           
          Color wild = new Color(PApplet.parseInt(getValue(j, i+2)));  //grabbing the color by paracing the text.
          //this is correct>>
          if (!drawHeat){
          ...
          break;
      case'G':
          command = PApplet.parseInt(getValue(j, i)*10);
...
public void scanGcode() {
  ...
  for (int j = iLine; j <= farToDraw; j++) {
        iprop = abs(PApplet.parseFloat(lineLimitHigh-lineLimitLow -j)/(lineLimitHigh-lineLimitLow));   //proportion of the way though, from 1 to 0, for displaying slice order
...
public void drawCommand() {  // draw based on globle values gathered earler
  noFill();
  stroke(128);
...
public void dline(float x1, float y1, float x2, float y2) {
  if (drawArrow && ( abs(x1 - x2) > 0.1f || abs(y1-y2) > 0.1f) ) {    // don't draw the arrow if it is a null line
    arrow(PApplet.parseInt(x1), PApplet.parseInt(y1), PApplet.parseInt(x1+(x2-x1)/2), PApplet.parseInt(y1+(y2-y1)/2) );
...
public void drawArc(float xC, float yC, float xS, float yS, float xF, float yF, boolean cw) { // draw a helical arc
...
public void sliderDist(int value){
  farToDraw = value;
  refresh = true;
}  
public void buttonDistUp(){
  if (farToDraw <=lineLimitHigh-1){farToDraw++; sliderDist(farToDraw);sliderDist.setValue(farToDraw);  }
}
public void buttonDistDown(){
  if (farToDraw >= lineLimitLow+1){farToDraw--;sliderDist(farToDraw);sliderDist.setValue(farToDraw);}
}



public void buttonShowLift(){
  drawLift = !drawLift;
  refresh = true;
}

public void buttonShowCut(){
  drawCut = !drawCut;
  refresh = true;
}
public void buttonShowHeat(){
 drawHeat = !drawHeat;
 refresh=true;
}
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "CylinDrawViewer" };
    if (passedArgs != null) {
