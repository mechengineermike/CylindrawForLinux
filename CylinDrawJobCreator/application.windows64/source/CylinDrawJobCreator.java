...
public class CylinDrawJobCreator extends PApplet {

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//INITIALIZATIONS TAB
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

 //used for pop up windowsa
...
float cupDiaMin = 80.00f; 
float cupDiaMax = 100.00f; 
float cupHeight = 160.00f;  //probable range 80 < H < 230
float stemHeight = 40.00f;
float toolDia = .500f; //could be anything here as it gets overwritten by whatever is in the settings file. 
...
float xOverlap = 50.0000f;//The percent overlap of fill lines to ensure that lines are dark & complete (20 means that a new fill line will overlap by 20%!!)   if it is 50 even it triggers a glitch in the library where it missses the center
float yOverlap = 50.0000f;//The percent overlap of fill lines to ensure that lines are dark & complete (20 means that a new fill line will overlap by 20%!!)
...
int  c0, c1;// c2;
int[]  colorArray;  //array of just the unique colors!
...
public void setup(){
  ...
  colorArray = new int[101]; //(0-100 inclusive)
  frameRate(60);
...
public void draw(){
    ...
    fill(0);
    imageMode(CENTER);
    float downscale = 2.2f;int imgpixelwidth = 2014;int imgpixelheight = 250;
    ...
    DisplayDataWindowY = PApplet.parseInt(height-680-2*plotyShift); //plotyShift //rect(xPosText -500, y-dy,xPosText+500,y+dy*20); //live stream system msg box window
    ...
    if (cupType ==0 || cupType ==2) { 
      ...
      if (cupType ==0){ 
       ...
       line( ((cupDiaMax-cupDiaMin)/2)*plotScale*PI+cupDiaMin*PI/4* plotScale,-1,((cupDiaMax-cupDiaMin)/2)*plotScale*PI+cupDiaMin*PI/4* plotScale,-0.1875f*stemHeight*plotScale); //base up left
       line( ((cupDiaMax+cupDiaMin)/2)*plotScale*PI-cupDiaMin*PI/4* plotScale,-1,((cupDiaMax+cupDiaMin)/2)*plotScale*PI-cupDiaMin*PI/4* plotScale,-0.1875f*stemHeight* plotScale); //base up right
       line( ((cupDiaMax-cupDiaMin)/2)*plotScale*PI+cupDiaMin*PI/4* plotScale,-0.1875f*stemHeight*plotScale,Xmax/2*plotScale-(0.03f*Xmax*plotScale),-0.25f*stemHeight*plotScale); //base canted  left
       line( ((cupDiaMax+cupDiaMin)/2)*plotScale*PI-cupDiaMin*PI/4* plotScale,-0.1875f*stemHeight*plotScale,Xmax/2*plotScale+(0.03f*Xmax*plotScale),-0.25f*stemHeight*plotScale); //base canted  right
       line( Xmax/2*plotScale-(0.03f*Xmax*plotScale),-0.25f*stemHeight*plotScale,Xmax/2*plotScale-(0.03f*Xmax*plotScale),-0.25f*stemHeight*plotScale-(stemHeight-(0.09375f*stemHeight)-0.375f*stemHeight)*plotScale); //stem left
       line( Xmax/2*plotScale+(0.03f*Xmax*plotScale),-0.25f*stemHeight*plotScale,Xmax/2*plotScale+(0.03f*Xmax*plotScale),-0.25f*stemHeight*plotScale-(stemHeight-(0.09375f*stemHeight)-0.375f*stemHeight)*plotScale); //stem right
       line( Xmax/2*plotScale-(0.03f*Xmax*plotScale),-0.25f*stemHeight*plotScale-(stemHeight-(0.09375f*stemHeight))*plotScale+(0.375f*stemHeight*plotScale),((cupDiaMax-cupDiaMin)/2)*plotScale*PI+cupDiaMin*PI/3* plotScale,-stemHeight*plotScale); //undrawable round bottom of glass left
       line( Xmax/2*plotScale+(0.03f*Xmax*plotScale),-0.25f*stemHeight*plotScale-(stemHeight-(0.09375f*stemHeight))*plotScale+(0.375f*stemHeight*plotScale),((cupDiaMax+cupDiaMin)/2)*plotScale*PI-cupDiaMin*PI/3* plotScale,-stemHeight*plotScale); //undrawable round bottom of glass left
    ...
    stroke(0);strokeWeight(0.7f); noFill(); //these are the seetings the preview shape will be drawn to
         ...
         arrListLocal = new ArrayList(arrList); //due to threadding to prevent sudden wipeout, this in part allows us to display teh arr as its being built
          ...
          if(!(bFastPreview && bSavingFile)){  //again this is an attempt to prevent freezing system while displaying a huge SVG job. If save takes too long people will exit system. 
              ...
              for (int p1 = 0; p1 < nve; p1++) {
                if (((Point) arrListLocal.get(p1)).z == -10.0f ){   //Removes 'travel' moves, -10z = beginning of drawn path, -20z end of path
                    beginShape();
                    vertex( ((Point) arrListLocal.get(p1-1)).x, ((Point) arrListLocal.get(p1-1)).y );
                }else if ( ((Point) arrListLocal.get(p1)).z == -20.0f) {
                    endShape();
                }else{
                  if (bMultiColor == true  ) {
    ...
    if (xWindow != width){ // if  frame.setSize(xWindow, yWindow);
      ...
      yWindow = height;
      plotyShift = PApplet.parseInt(height/2-Ymax*plotScale/2-height+62+550);
      setButtons();      
    }
...
public void saveFile(){
    if (!bPaid){
      ...
      svg.stroke(0); svg.strokeWeight(0.2f); //////FUCKT THIS svg.background(255); MAKE THE RESULT APPEAR SOLID BLACK WHEN RELOADED IN RUNMODE //used for prev, white background
      svg.scale(1,-1);
      ...
      int localNew = color(2);
      int localOld = color(255);
      ...
      if (!bFastPreview){             
            for (int p = 0; p < arrList.size(); p++) {
                ...
                if (((Point) arrList.get(p)).z == -10.0f ){   // -10z = beginning of path, -20z end of bath
                ...
                }else if ( ((Point) arrList.get(p)).z == -20.0f) {
                    if(bStarted){svg.endShape(); bStarted = false;}
            ...
            if (cupType ==0 || cupType ==2  ) {  //This draws the boundary (RED lines) for prev
               ...
               svg.line( ((cupDiaMax-cupDiaMin)/2)*PI+cupDiaMin*PI/4,-1*-1,((cupDiaMax-cupDiaMin)/2)*PI+cupDiaMin*PI/4,-0.1875f*stemHeight*-1); //base up left
               svg.line( ((cupDiaMax+cupDiaMin)/2)*PI-cupDiaMin*PI/4,-1*-1,((cupDiaMax+cupDiaMin)/2)*PI-cupDiaMin*PI/4,-0.1875f*stemHeight*-1); //base up right
               svg.line( ((cupDiaMax-cupDiaMin)/2)*PI+cupDiaMin*PI/4,-0.1875f*stemHeight*-1,Xmax/2-(0.03f*Xmax),-0.25f*stemHeight*-1); //base canted  left
               svg.line( ((cupDiaMax+cupDiaMin)/2)*PI-cupDiaMin*PI/4,-0.1875f*stemHeight*-1,Xmax/2+(0.03f*Xmax),-0.25f*stemHeight*-1); //base canted  right
               svg.line( Xmax/2-(0.03f*Xmax),-0.25f*stemHeight*-1,Xmax/2-(0.03f*Xmax),(-0.25f*stemHeight-(stemHeight-(0.09375f*stemHeight)-0.375f*stemHeight))*-1 ); //stem left
               svg.line( Xmax/2+(0.03f*Xmax),-0.25f*stemHeight*-1,Xmax/2+(0.03f*Xmax),(-0.25f*stemHeight-(stemHeight-(0.09375f*stemHeight)-0.375f*stemHeight))*-1 ); //stem right
               svg.line( Xmax/2-(0.03f*Xmax),(-0.25f*stemHeight-(stemHeight-(0.09375f*stemHeight))+(0.375f*stemHeight))*-1,((cupDiaMax-cupDiaMin)/2)*PI+cupDiaMin*PI/3,-stemHeight*-1); //undrawable round bottom of glass left
               svg.line( Xmax/2+(0.03f*Xmax),(-0.25f*stemHeight-(stemHeight-(0.09375f*stemHeight))+(0.375f*stemHeight))*-1,((cupDiaMax+cupDiaMin)/2)*PI-cupDiaMin*PI/3,-stemHeight*-1); //undrawable round bottom of glass left
            ...
            int textSize = PApplet.parseInt(Ymax/20);
            ...
            if (cupType==2){
              ...
              svg.text(3.14f*cupDiaMax,5,-Ymax+(iliney)*spacing+offsett);iliney++;
           ...
           // PShape thing = loadShape("temp.svg");
            ...
            timeEst = (gcodecommandlist.length-1) *2.5f/20/60;  // estimate 2 sec per 20 lines, 60 seconds per minute
...
public void mouseWheel(MouseEvent event){
  ...
  if (bSavingFile == false) { //dont interrupt
      ...
      if (plotScale <.1f){ plotScale =.1f; }
      plotyShift = PApplet.parseInt(0+plotScale*Ymax*.5f-height/2+200);
      shapeLoaded = false; 
  }
}
     ...
     // } 
      ...
      for (int lineCount = 0; lineCount<linesRaw.length - 1; lineCount++) {   // PARSE gcode & remove everything upto and including "<!-- BEGIN"
          ...
          if (linesRaw[lineCount].contains("Unique Colors: ")) {
             ...
             //DisplayData("Found      =" +rawValue);
             int iNumColors = PApplet.parseInt(Float.parseFloat(rawValue));
             if ( iNumColors>2 ){
               bMultiColor = true;
             }else{
               bMultiColor = false;
...
public void launchViewer(File selectedFile){
   String newPath = sketchPath(); //sketch patch expludes the name of this sketch, it is just the folders leadin gup to it and the master group folder is "CylinDraw" Sub folders & programs have set names.
...
public void fileCopy(){  //If user picks file not located in processing parent folder, we copy it temporarily, then delete that copy on proper exit.  
...
public void displayZero(){  // draw the zero axis X & Y lines 
    ...
    for(int i = 0 ; i < Xmax ; i++){ //horizontal zero reference line
      line( (PApplet.parseFloat( i) * plotScale),0, ( PApplet.parseFloat(i) * plotScale),- 3);  //3 is the LONG Line width. 
      if((i % 10) == 0) line( (PApplet.parseFloat( i) * plotScale), 0, ( PApplet.parseFloat(i) * plotScale), - 8); //8 is the tick mark length 
    } 
     for(int i = 0 ; i < Ymax ; i++){ //vertical zero reference line
      line(0, - (PApplet.parseFloat(i)* plotScale), 3, - (PApplet.parseFloat(i)* plotScale) ); //
      if((i % 10) == 0) line(0 , - (PApplet.parseFloat(i)* plotScale),  8, - (PApplet.parseFloat(i)* plotScale) ); //
    }  
}//end of drawZero()


public void DisplayData(String in){ //Used to show serial data within the user application window
   // println(in); // Println IS ONLY USEFUL FOR DISPLAYING IN PROCSSINGS COMMAND WINDOW BELOW. It is darn slow and unecessary for displaing within Draw so its commented out.
    ...
    int lineLimit = PApplet.parseInt((DisplayDataWindowY-17/5)/25);
...
public void checkLicense(String inputEmail,String inputKey) {
...
public void enterCupDimensions2(){
  ...
  if (option == 0 || option == 3){// JOptionPane.OK_OPTION){
      ...
      try{ 
        ...
        // if (value7 != null){ bMultiColor = Integer.parseInt(value7); } 
         ...
         plotScale = 2.3f*160/cupHeight;  //experimentally determined to procude a default scale that fits everything on screen. The scale can still adjust with the mouse.      
      }catch(NumberFormatException ne){
...
public void loadSettings(String sCupToolFile){ //open CupTool_LastUsed 'settings.txt'. Collect info and update dimensions. Do this when first opening this program.
    ...
    if (file.exists()) {
     try{
        ...
        if (lines != null){
          ...
          cupType = PApplet.parseInt(lines[2]);//The zero line is the header line so we skip it. & there are note lines so we skip every other line 
          cupDiaMax = PApplet.parseFloat(lines[4]);
          cupDiaMin= PApplet.parseFloat(lines[6]);
          cupHeight = PApplet.parseFloat(lines[8]);
          if (!bLocallyMade){ toolDia = PApplet.parseFloat(lines[10]); }
          stemHeight = PApplet.parseFloat(lines[12]);
...
public void buttonUpdate(){ enterCupDimensions2(); shapeLoaded = false; } 
...
public void exportSelected(File selection) {
    if (selection != null){
...
public void buttonRotateLeft(){     if (shapeLoaded == true){shapeLoaded = false; rotateAngle -= 5*PI/180; DisplayData( " Drawing Rotated -5 deg. Total angle is: " + rotateAngle*180/PI + " degrees");}}

public void buttonScaleUp(){    if (shapeLoaded == true){shapeLoaded = false; imageScaleAdjust = imageScaleAdjust+ (imageScale+imageScaleAdjust)*0.05f; DisplayData( "Drawing Scaled Larger");}}

public void buttonScaleDown(){    if (shapeLoaded == true){shapeLoaded = false; imageScaleAdjust = imageScaleAdjust- (imageScale+imageScaleAdjust)*0.05f; DisplayData( "Drawing Scaled Smaller");}}

public void buttonRotateRight(){    if (shapeLoaded == true){shapeLoaded = false; rotateAngle += 5*PI/180; DisplayData( " Drawing Rotated +5 deg. Total angle is: " + rotateAngle*180/PI + " degrees");}}

public void buttonMoveLeft(){ if (shapeLoaded == true){shapeLoaded = false; xShift--; DisplayData( "You clicked the 'LEFT arrow Key'. Drawing Moved LEFT");}} 

public void buttonMoveUp(){  if (shapeLoaded == true){shapeLoaded = false; yShift--; DisplayData( "You clicked the 'UP Arrow Key'. Drawing Moved UP");} }

public void buttonMoveDown(){  if (shapeLoaded == true){shapeLoaded = false; yShift++; DisplayData( "Drawing Shifted 'DOWN Arrow Key' Drawing Moved Down");}}

public void buttonMoveRight(){  if (shapeLoaded == true){shapeLoaded = false; xShift++ ;DisplayData( "You clicked the 'RIGHT Arrow Key'. Drawing Moved RIGHT");}}
...
public void buttonProgCreate(){  DisplayData("You are currently using Creation Mode."); }


public void buttonProgRun(){
...
public float colorDist(int c1,int c2){ //min value is 0 max is 441.673 when colors are constrained between 0-255
  return  dist(red(c1),green(c1),blue(c1),red(c2), green(c2),blue(c2));
} 



public void inVert(RShape s ) {  //Convert all shape colors to black
  RShape[] ch; // children
  int n, i; 
  n = s.countChildren();
  if (n > 0) {
...
public void exVert(RShape s, int c, boolean bPerimeter, boolean bFill) {  // Inputs: The RShape you want to parse, a color, bool parse outlines, bool pase infill)
  RShape[] ch; // children
  int n, i; 
  n = s.countChildren();
 ...
 // println("color #" + iColorIndex + " is " + c0);
  if (n > 0) {
     ...
     if (bPerimeter){
         penIsDown = true;//just assume it is down before doing a perimeter
          if (penIsDown){ //moving from something else to a fill, ensure pen is up
              arrList.add(new Point(0,0, -20.0f, c)); //LIFT
              penIsDown = false;  
          }
            ...
            for (a=0; a<ccc; a++) { //perimeter of local shape
              if (pa[a] !=null)for (b=0; b<pa[a].length; b++) {
                if (b==0) {
                  ...
                  arrList.add(new Point(0, 0, -10.0f, c));  // DROP z = -10z :beginn of a drawing path (drop pen)
                  penIsDown = true;
                } else {
                  if (b==pa[a].length-1) {
                    ...
                    arrList.add(new Point(pa[a][b].x, pa[a][b].y, 0.0f, c));
                  }
                }
              }
    ...
    if ( bFill && !bLocallyMade ){ //Infill of THIS localshape (also only process this on a save file cycle because its slow!
          ...
          float dotArrayWidth = abs(LocalbottomRight.x - LocaltopLeft.x)+.5f;
          ...
          float dotArrayHeight  = abs(LocalbottomRight.y - LocaltopLeft.y)+.5f;
         ...
         // dotArrayY = new float[numArrayRows][numArrayCols];  
          ...
          for (float cc = 0; cc <numArrayCols; cc++) {//ONE SHAPE! //Starting at the top LEFT X point (c) on the bounding box, iterate through columns for the given row.
              ...
              float xPositionConst =  (float)(cc)*toolDia*(100.0f-xOverlap)/100.0f +(LocaltopLeft.x); //constant for this loop of cc, saves some calculations
              ...
              float ymaxxy = ((Ymax - (float)(numArrayRows)*toolDia*(100.0f-yOverlap)/100.0f)) + (LocalbottomRight.y) - (Ymax-dotArrayHeight);
                  ...
                  if (swoopUp ){//&& countNodes > 1
                    for (int pnt=0; pnt<countNodes; pnt++) {
                     ...
                     // if (int(pnt) %2==0  ){ arrInit[int(pnt)] = false;}
                      ...
                      if (pnt % 2 ==0 && abs(abs(tempListY[pnt]) - abs(oldYdown[PApplet.parseInt(pnt/2)])) > toolDia ) {arrInit[PApplet.parseInt(pnt/2)] = false;} //Transfer move-CHECK Y. if transfer move is greater than the actual move
                      if (pnt % 2 ==0 && abs(tempListX[pnt]) >(abs(oldXdown[PApplet.parseInt(pnt/2)])+ 1.00f*toolDia*(100.0f-xOverlap)/100.0f   )) {arrInit[PApplet.parseInt(pnt/2)] = false; }//Transfer move-CHECK X
                      //reversing algorythm
                      ...
                      if (arrInit[PApplet.parseInt(pnt/2)] == false ) { //(initilalize the use of this arraylist if it is new)
                        //if (penIsDown){ //moving from something else to a fill, ensure pen is up
                          arrSubLists[PApplet.parseInt(pnt/2)].add(new Point(0,0, -20.0f, c)); //LIFT
                          penIsDown = false;  
                        //}
                        arrSubLists[PApplet.parseInt(pnt/2)].add(new Point(tempListX[pnt], tempListY[pnt], 0.0f, c)); 
                        arrSubLists[PApplet.parseInt(pnt/2)].add(new Point(0,0, -10.0f, c)); //-10z DROP
                        penIsDown =true;
                        arrSubLists[PApplet.parseInt(pnt/2)].add(new Point(tempListX[pnt], tempListY[pnt], 0.0f, c)); //go to the same point as above for svg display reasons
                        arrInit[PApplet.parseInt(pnt/2)] = true; 
                        penIsDown = true;
                      }else{ arrSubLists[PApplet.parseInt(pnt/2)].add(new Point(tempListX[pnt], tempListY[pnt], 0.0f, c)); }
                      if (pnt % 2 != 0){oldYup[PApplet.parseInt(pnt/2)] = tempListY[pnt]; oldXup[PApplet.parseInt(pnt/2)] = tempListX[pnt];}  // When moving UP the even numbered points are the target location of the transfer, and the odd is the end of the line
                      ...
                      for (int pnt=countNodes-1; pnt>=0; pnt--) {
                       ...
                       // if (pnt %2==0 ){ arrInit[int(pnt/2)] = false;}
                        ...
                        if (pnt % 2 !=0 && abs(abs(tempListY[pnt]) - abs(oldYup[PApplet.parseInt(pnt/2)]))>toolDia ) {arrInit[PApplet.parseInt(pnt/2)] = false;}
                        if (pnt % 2 !=0 && abs(tempListX[pnt]) >  (abs(oldXup[PApplet.parseInt(pnt/2)])+ 1.00f*toolDia*(100.0f-xOverlap)/100.0f      ) ) { arrInit[PApplet.parseInt(pnt/2)] = false;}                     //    println("new x pos is " + abs(tempListX[pnt]));                         //   println("  old x pos is " + abs(oldXup[int(pnt/2)]));
                        //reversing algorythm 
                        ...
                        if (arrInit[PApplet.parseInt(pnt/2)] == false ) { //(initilalize the use of this arraylist if it is new)
                          ...
                          //}
                          arrSubLists[PApplet.parseInt(pnt/2)].add(new Point(tempListX[pnt], tempListY[pnt], 0.0f, c));
                          arrSubLists[PApplet.parseInt(pnt/2)].add(new Point(0,0, -10.0f, c)); //-10z drop pen-
                          penIsDown = true;
                          arrSubLists[PApplet.parseInt(pnt/2)].add(new Point(tempListX[pnt], tempListY[pnt], 0.0f, c)); //go to the same point as above for svg display reasons
                          arrInit[PApplet.parseInt(pnt/2)] = true; 
                          
                        } else { arrSubLists[PApplet.parseInt(pnt/2)].add(new Point(tempListX[pnt], tempListY[pnt], 0.0f, c));}
                        if (pnt % 2 ==0){oldYdown[PApplet.parseInt(pnt/2)] = tempListY[pnt]; oldXdown[PApplet.parseInt(pnt/2)] = tempListX[pnt];}// When moving down the odd numbered points are the target location of the transfer, and the even is the end of the line
...
public void hone(){  //final code clean up prior to save 
  ...
  //create lists sorted by color 0-100 = 101 pieces
   ...
   // colorArray = reverse(colorArray); //this works but you have to move black to the end...!
    ...
    int[]  tempColorArray; 
    tempColorArray = new int[101]; //(0-100 inclusive)
                     ...
                     for (int groupId =0; groupId<=groupIndex;groupId++){
                         if (arrInit[groupId]){ //if(arrGroupLists[groupId].size() <=0){arrInit[holdId] = false; }else{
                              ...
                              if ( ( abs(xFirst-xLast) + abs(yFirst-yLast) )  <( yDist + xDist ) ) {//     abs(xFirst-xLast) < xDist && abs(yFirst-yLast)  < yDist ) {    //evaluate seperately to avoid local maximum                         
                                  ...
                                  holdId = groupId; 
                                  if ( xDist<=1.5f*toolDia*fOverlap/100 && + yDist<=1.5f*toolDia*fOverlap/100){bSkipLift =true;}
                              }
                          }
                      }
                      float xDist2=9999;
                      ...
                      if(holdId !=prevHoldID){
                          if ( xDist<=1.5f*toolDia*fOverlap/100 && + yDist<=1.5f*toolDia *fOverlap/100 ){bSkipLift =true; } //// 1x*fOverlap/100 (was in release v2.0), switched to 1.5 to include hypotenuece of diagonal moves, reduce overall jumps!               //println(" JUMP SKIPPED!!!! x= " +xFirst+"   y = "+yFirst );
                          //1.01*toolDia*(100.0-xOverlap)/100.0 
                          if (bSkipLift == false) {arrList.add(new Point(0,0, -20.0f,c));  }//LIFT println("added lift ");  
                           arrList.addAll(arrGroupLists[holdId]);
                          
                          if (bSkipLift == false) {arrList.add(new Point(0,0, -10.0f,c));  }//DROP println("added lift ");
                          arrInit[holdId] = false;
                      }prevHoldID = holdId;
                      groupsAdded++;
  ...
  colorArray = new int[101]; //I guess Im resetting the  color array here because Im already done using it from above and i need to reset it for next time...
...
class Point {   //'travel' moves, -10z = beginning of drawn path, -20z end of path
  ...
  int clr;
 // float strk;
  Point(float x, float y, float z, int clr) { 
    this.x = x;
    this.y = y;
    this.z = z;
    this.clr = clr;
...
public void printColors (int rgb) {
...
//multicolors were here
  ...
  buttonScaleDown = gui.addButton("buttonScaleDown").setCaptionLabel("Smaller").setPosition(Xpos, Ypos+ySpacing*2).setColorValue(100).setColorCaptionLabel(color(0)).setColorLabel(255).setSize(sizeX,sizeY).setFont(font); buttonScaleDown.hide();
    ...
    sizeX =100; 
    sizeY =26;ySpacing = sizeY +5; 
  toggleMultiColor = gui.addToggle("toggleMultiColor").setCaptionLabel("MonoColor   MultiColor").setPosition(Xpos, Ypos).setMode(ControlP5.SWITCH).setColorCaptionLabel(color(255)).setSize(PApplet.parseInt(sizeX*1.5f),sizeY).setFont(font);toggleMultiColor.hide();
    gui.getController("toggleMultiColor").getCaptionLabel().align(CENTER,CENTER);
    Ypos =  Ypos + ySpacing;
  toggleOutline = gui.addToggle("toggleOutline").setCaptionLabel("   Normal        Blk-Outline").setPosition(Xpos, Ypos).setMode(ControlP5.SWITCH).setColorCaptionLabel(color(255)).setSize(PApplet.parseInt(sizeX*1.5f),sizeY).setFont(font);toggleOutline.hide();
    gui.getController("toggleOutline").getCaptionLabel().align(CENTER,CENTER);
    Ypos =  Ypos + ySpacing;   
  toggleFill = gui.addToggle("toggleFill").setCaptionLabel("No-Fill            Fill       ").setPosition(Xpos, Ypos).setMode(ControlP5.SWITCH).setColorCaptionLabel(color(255)).setSize(PApplet.parseInt(sizeX*1.5f),sizeY).setFont(font); toggleFill.hide();
...
public String cleanString(String sCleanMe){
...
public void logHold(String log){//append a string to the soft log
    sLog = sLog + log;
}

public void logWrite(boolean bAppend){//commit softlog to hardlog. (without append it will fully overwright but it will take longer.)
...
public void logRead(){//overwrite softlog with hardlog  
    sLog ="";
    String[] sLogArray = loadStrings("\\system\\LogJobCreator.txt");
    sLog = join(sLogArray,"\n");
}
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "CylinDrawJobCreator" };
    if (passedArgs != null) {
