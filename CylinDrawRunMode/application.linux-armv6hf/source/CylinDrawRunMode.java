...
public class CylinDrawRunMode extends PApplet {
...
float previewY=207.5f;//yWindow/4.5;
...
public void isPortActive() {
  ...
  if (bConnected ) {//verify that it is still connected to same port
    ...
    //
    for (int jj=PApplet.parseInt(random(0, ports.length)); jj<ports.length; jj++) { //this part is important. For every serial device found we hold its portname and check if it connects at our unusual baud rate via openSerialPor. This should work on any platform...
...
public void selectSerialPort() { //Use selectSerialPor OR isPortActiv but not both (need a settings file2 and have this be an option)
...
public void draw() {
  ...
  float calc= ((timeEstCore * (100/iSpeed + 100/iSpeedJog)/2 * (1000/iAccel + 1000/iAccelJog)/2 -(timeCount/1000/60))  *1.65f/20/60);
  ...
  DisplayDataWindowY = PApplet.parseInt(height-(y-dy-2)-30);  
  rect(5, y-dy-2, width-5, y+DisplayDataWindowY); //live system msg box window
  fill(0);

  text("--System Messages--", 20, y); 
  y += dy*.7f;   //REDUCE TEXT SIZE & DY RIGHT HERE TO GET MORE COMMANDS VISIBLE ON SCREEN
  textSize(16);
  text(liveData, 12, y);
  textSize(20);
  ...
  if (bPreviewLoaded == true) {  //This will rescale the preview
    ...
    if (  h >550/2) {//||  w > 550 //SHRINK IT
      ...
      previewScale = previewScale*.85f;
      //if (bVerbose)print(previewScale);
      loadPreview();
    } else if (  h <400/2) {//||w < 400   //GROW IT
      text("(Loading job preview now! (scaling the preview...)", width/2, 200);
      previewScale = previewScale*1.15f;
      //if (bVerbose)print(previewScale);
      loadPreview();
    } else {   //  */
      Rpreview.draw();
...
public void DisplayData(String in) { //Used to show serial data within the user application window
  ...
  int lineLimit = PApplet.parseInt(DisplayDataWindowY/26.5f);
...
public void keyPressed() { //Keyboard Commands
  if (key == ESC) { 
    key = 0; 
    buttonExit();
  }
...
public void fileSelected(File selection) { //LOAD THE CODE FROM A FILE
...
public void fileCopy() {  //If user picks file not located in processing parent folder, we copy it!
  ...
  bFastPreview=false;
  if (fileSize>(1000000*10/6.5f)){ //ratio derived from experiment job is approximated 10/6.5 times bigger than svg
        bFastPreview=true;
...
public void loadPreview() {
  if (bFileLoaded == true) {
    ...
    previewY=207.5f;////;height/4.5;//offset fixed distance
...
public void sendSpeed() { //value is between 10 & 100
  if (bConnected) {
    queCmd("G50 S" + iSpeed +"\n", false);//
  }
}
public void sendAccel() { //value is between 10 & 100
  if (bConnected) {
    queCmd("G51 A" + iAccel +"\n", false);
  }
}
public void sendSpeedJog() { //value is between 10 & 100
  if (bConnected) {
    queCmd("G60 S" + iSpeedJog +"\n", false);
  }
}
public void sendAccelJog() { //value is between 10 & 100
  if (bConnected) {
...
public void toolChange(String colorstring ) { // make sure you are sending everythign about the color line except the Q ( do include the sign though!)
  buttonPause();
...
public void buttonProgCalibrate() {
...
public void serialEvent(Serial p) { //This function displays serial data returned from the arduino. Is called when recieving special characters
  bProvenConnection = true;
  try {    
    if (checkComplete()) {
      bOKtoSend = true;
...
public void setButtons() {
  ...
  buttonSpeed  = gui.addButton("buttonSpeed").setCaptionLabel("Update Speed").setPosition(Xpos, Ypos).setColorValue(100).setColorCaptionLabel(color(0)).setColorLabel(255).setSize(sizeX, PApplet.parseInt(sizeY)).setFont(font).hide();
  Ypos = Ypos+ySpacing;
  ...
  buttonRotate180 = gui.addButton("buttonRotate180").setCaptionLabel("Rotate Cup 180Deg\n    (untracked)").setPosition(Xpos, Ypos).setColorValue(100).setColorCaptionLabel(color(0)).setColorLabel(255).setSize(sizeX, PApplet.parseInt(sizeY)).setFont(font).hide();
  Ypos = Ypos+ySpacing;
  ...
  buttonJumpTo = gui.addButton("buttonJumpTo").setCaptionLabel("Jump to Line:").setPosition(Xpos, Ypos).setColorValue(100).setColorCaptionLabel(color(0)).setColorLabel(255).setSize(sizeX, PApplet.parseInt(sizeY)).setFont(font).hide();
...
public void queCmd(String sWrite, boolean bCut) {//add to top of stack (myPort command). Que will auto empyt
  if (bCut) {
...
public void queRun() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "CylinDrawRunMode" };
    if (passedArgs != null) {
